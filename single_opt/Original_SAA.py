...
for m in range(MS):
    # generate samples
    while total_demand > 40000 or total_demand < 30000:
        sample = np.random.choice(NS, SS, replace=False)
        # generate corresponding demand
        demand_sample = np.zeros((SS, IS))
        pr_sample = np.ones(SS) / SS
        for s in range(SS):
            for j in range(IS):
                demand_sample[s, j] = demand[int(sample[s]), j]
        total_demand = sum(sum(demand_sample))
    for j in range(IS):
    ...
    for s in range(SS):
        for a in range(AS):
            for j in range(IS):
                D_sample[s, a, j] = D[int(sample[s]), a, j]  # Fixed the indexing here
