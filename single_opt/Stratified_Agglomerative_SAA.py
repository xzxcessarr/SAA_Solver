# -*- coding: utf-8 -*-
script_name = 'Stratified_Agglomerative'
...
# 计算距离矩阵
distance_matrix = pairwise_distances(demand, metric='euclidean')

# samples = []
sum_sample = np.zeros((MS, IS))

# cluster = AgglomerativeClustering(n_clusters=SS_SAA, linkage='ward').fit(demand)
cluster = AgglomerativeClustering(n_clusters=SS_SAA, linkage='average').fit(distance_matrix)
