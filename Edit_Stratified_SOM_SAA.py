...
num_unique_labels = len(cluster_labels)
...
for m in range(MS):
    ...
    standard = np.zeros((num_unique_labels, 1))
    temp_num = np.zeros((num_unique_labels, 1))
    for i in np.unique(cluster_labels):  # 直接在for循环中使用np.unique(cluster_labels)
        cluster_each = np.argwhere(cluster_labels == i)[:, 0]  # 获取索引数组
        demand_sample = demand[cluster_each, :]  # 直接使用索引数组
        # determine scenario number for each sample by standard deviation
        mean_sample = np.mean(demand_sample, axis=0)
        standard[i] = np.sqrt(np.sum(np.square(demand_sample - mean_sample)) / len(cluster_each))
        temp_num[i] = standard[i] * len(cluster_each)

    for i in np.unique(cluster_labels):  # 直接在for循环中使用np.unique(cluster_labels)
        cluster_each = np.argwhere(cluster_labels == i)[:, 0]  # 获取索引数组
        pick_num = np.rint(num_unique_labels * len(cluster_each) * standard[i] / np.sum(temp_num))
        pick_num = int(pick_num)  # 转换为整数
        
        if pick_num == 0:
            continue
        elif pick_num == 1:
            temp = np.random.choice(cluster_each, 1, replace=False)
            sample.append(temp[0])
        else:
            temp = np.random.choice(cluster_each, pick_num, replace=False)
            sample.extend(temp)  # 可以使用extend来附加数组中的多个元素
