# -*- coding: utf-8 -*-
script_name = 'Stratified_SpectralClustering'
...
# 假设'demand'是需要聚类的数据，且已经被聚类
pca = PCA(n_components=2)  # 降至二维
demand_transformed = pca.fit_transform(demand)
...
for m in range(MS):
    ...
    for i, label in enumerate(np.unique(cluster.labels_)):
        plt.scatter(
            demand_transformed[cluster.labels_ == label, 0],
            demand_transformed[cluster.labels_ == label, 1],
            label=f'Cluster {i}'
        )
    ...
    # 保存图片到指定目录
    plt.savefig(os.path.join(config.Graphs_sample_save_directory, f'{script_name}_sample_{m+1}.jpg'), format='jpg')
